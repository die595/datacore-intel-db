...
function ChartCard({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="bg-[#0D1321]/80 border border-cyan-900/20 rounded-lg p-3 flex flex-col">
      <h3 className="text-cyan-400 text-[10px] font-bold tracking-widest uppercase mb-2">{title}</h3>
...
export default function ChartsPanel() {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-2 p-2">
      ...
      <ChartCard title="Desglose por Tipología">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={data.tipologias} layout="vertical" margin={{ left: 10, right: 10, top: 5, bottom: 5 }}>
            <XAxis type="number" tick={{ fontSize: 10, fill: '#64748b' }} tickLine={false} />
            <YAxis type="category" dataKey="name" tick={{ fontSize: 9, fill: '#94a3b8' }} tickLine={false} width={90} />
            <Tooltip contentStyle={{ background: '#111827', border: '1px solid rgba(0,217,255,0.3)', borderRadius: 8, fontSize: 11 }} />
      ...
      <ChartCard title="Municipio">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={data.municipios} margin={{ left: 5, right: 5, top: 5, bottom: 40 }}>
            <XAxis dataKey="name" tick={{ fontSize: 8, fill: '#94a3b8' } as any} tickLine={false} height={50} interval={0} angle={-45} />
      ...
      {/* Fenomenos pie chart */}
      <ChartCard title="Fenómeno Criminalidad">
        <ResponsiveContainer width="100%" height="100%">
          <PieChart>
            <Pie
              data={data.fenomenos}
              ...
              innerRadius={30}
              dataKey="value"
              label={({ name, percent }: any) => `${(name ?? '').substring(0, 12)} ${((percent ?? 0) * 100)?.toFixed?.(0) ?? '0'}%`}
              labelLine={false}
              style={{ fontSize: 8 }}
            >
      ...
      {/* Radar chart */}
      <ChartCard title="Análisis Radar Fenómenos">
        <ResponsiveContainer width="100%" height="100%">
          <RadarChart data={data.fenomenoRadar} cx="50%" cy="50%" outerRadius={60}>
            <PolarGrid stroke="#1e293b" />
            <PolarAngleAxis dataKey="name" tick={{ fontSize: 7, fill: '#94a3b8' }} />
      ...
      <div className="md:col-span-2 xl:col-span-4">
        <ChartCard title="Línea Temporal de Incidentes">
          <ResponsiveContainer width="100%" height={180}>
            <LineChart data={data.timeline} margin={{ left: 10, right: 10, top: 5, bottom: 25 }}>
              <XAxis dataKey="name" tick={{ fontSize: 9, fill: '#94a3b8' } as any} tickLine={false} height={40} interval="preserveStartEnd" angle={-45} />
              <YAxis tick={{ fontSize: 10, fill: '#64748b' }} tickLine={false} />
              <Tooltip contentStyle={{ background: '#111827', border: '1px solid rgba(0,217,255,0.3)', borderRadius: 8, fontSize: 11 }} />
              <Line type="monotone" dataKey="value" stroke="#00D9FF" strokeWidth={2} dot={{ fill: '#00D9FF', r: 2 }} activeDot={{ r: 5, fill: '#FF4444' }} />
            </LineChart>
          </ResponsiveContainer>
        </ChartCard>
      </div>
